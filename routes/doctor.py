...
from models.models import User, Alert, Report, Conversation, Message, ConnectionRequest
...
def get_available_patients():
    ...
    all_patients = User.query_by(role='patient')
    result = []
    for p in all_patients:
        # Check if already connected or request pending
        existing_requests = ConnectionRequest.query_by(doctor_id=current_user.id, patient_id=p.id)
        existing = None
...
def send_connection_request(patient_id):
    ...
    existing_requests = ConnectionRequest.query_by(
...
def get_patient_connection_requests():
    ...
    requests_list = ConnectionRequest.query_by(patient_id=current_user.id, status='pending')
    # Sort by created_at descending
    requests_list.sort(key=lambda r: r.created_at if r.created_at else datetime.min, reverse=True)
...
def respond_to_connection(request_id):
    ...
    conn_req = ConnectionRequest.get_by_id(request_id)
    ...
    if action == 'accept':
        ...
        doctor = User.get_by_id(conn_req.doctor_id)
        return jsonify({
            'message': f'Connected with Dr. {doctor.username if doctor else "Unknown"}',
...
@doctor_required
def get_patients():
    patients = User.query_by(assigned_doctor_id=current_user.id, role='patient')
    patient_list = []
    for p in patients:
        reports = Report.query_by_user_ordered(p.id, ascending=True)
        latest_report = reports[-1] if reports else None
        
        all_convs = Conversation.query_by(user_id=p.id)
        active_convs = sum(1 for c in all_convs if c.is_active)
        total_convs = len(all_convs)
        
        unread_alerts = len(Alert.query_by(user_id=p.id, doctor_id=current_user.id, is_read=False))
...
def get_patient_reports(patient_id):
    ...
    reports = Report.query_by_user_ordered(patient_id, ascending=False)
    return jsonify({
...
def get_patient_conversations(patient_id):
    ...
    convs = Conversation.query_by_user_ordered(patient_id)
    return jsonify({
        'patient': patient.to_dict(),
...
def get_patient_analytics(patient_id):
    ...
    reports = Report.query_by_user_ordered(patient_id, ascending=True)

    # All messages with sentiment scores
    conversations = Conversation.query_by(user_id=patient_id)
    all_messages = []
    crisis_count = 0
    total_messages = 0
    sentiment_sum = 0
    ...
    # Count crisis alerts
    crisis_count = Alert.count_by(user_id=patient_id, alert_type='crisis')
...
@doctor_required
def get_alerts():
    alerts = Alert.query_by_doctor_ordered(current_user.id)
...
def get_unread_alerts():
    alerts = Alert.query_by(doctor_id=current_user.id, is_read=False)
    # Sort by created_at descending
    alerts.sort(key=lambda a: a.created_at if a.created_at else datetime.min, reverse=True)
